    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args != null && args.Length > 0)
            {
                ...
                {
                    Console.WriteLine($"Command line argument could not be parsed as an IP address \"{args[0]}\"");
            ...
            sipTransport.SIPTransportRequestReceived += async (SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint, SIPRequest sipRequest) =>
            {
                try
                {
                    if (sipRequest.Method == SIPMethodsEnum.INVITE)
                    {
                        ...
                        if (offerSdp.Media.Any(x => x.Media == SDPMediaTypesEnum.audio && x.MediaFormats.Any(y => y.Key == (int)SDPWellKnownMediaFormatsEnum.PCMU)))
                        {
                            Console.WriteLine($"Client offer contained PCMU audio codec.");
            ...
            Task.Run(() =>
            {
                ...
                {
                    Console.WriteLine($"Exception Key Press listener. {excp.Message}.");
                }
            });
        ...
        private static async Task Hangup(SIPServerUserAgent uas)
        {
            try
            {
                if (uas?.IsHungup == false)
                {
                    ...
                    Console.WriteLine("Waiting 1s for call to hangup...");
                    await Task.Delay(1000);
                }
            }
            catch (Exception excp)
            {
                Console.WriteLine($"Exception Hangup. {excp.Message}");
            }
        }
